...
<div class="flex h-screen overflow-hidden">
    ...
    <div class="w-96 border-l p-6 bg-white flex flex-col">
        ...
        <div class="border-t pt-4">
            <div class="text-sm mb-2">Subtotal: Rp {subtotal.toLocaleString()}</div>
            {#if taxConfig}
                <div class="text-sm mb-2">{taxConfig.name} ({taxConfig.rate}%): Rp {taxAmount.toLocaleString()}</div>
            {/if}
            <div class="font-bold text-lg mb-2">Total: Rp {total.toLocaleString()}</div>
